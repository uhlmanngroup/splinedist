...
def draw_polygons(coord, score, poly_idx, grid=(1, 1), cmap=None, show_dist=False):
    ...
    return _draw_polygons(
        polygons=coord[poly_idx[:, 0], poly_idx[:, 1]],
...
def _draw_polygons(
    polygons, points=None, scores=None, grid=None, cmap=None, show_dist=False
):
    """
    ...
    for point, poly, score, c in zip(points, polygons, scores, cmap.colors[1:]):
        ...
        if show_dist:
            ...
            plt.gca().add_collection(
                LineCollection(dist_lines, colors=c, linewidths=0.4)
            )
