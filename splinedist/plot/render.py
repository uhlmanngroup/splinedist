...
def random_hls(n=2 ** 16, h0=0.33, l0=(0.8, 1), s0=(0.5, 0.8)):
    """
    h0 = 0 -> red
    h0 = 0.33 -> green
    h0 = 0.66 -> blue
...
def match_labels(y0, y):
    ...
    res = matching(y0, y, report_matches=True, thresh=0.1)
    ...
    leftover_labels = leftover_labels.union(
        set(
            np.max(ind_matched0)
            + 1
            + np.arange(len(ind_unmatched) - len(leftover_labels))
        )
    )
...
def render_label_pred(
    ...
    # green
    cmap_tp, cmap_border_tp = gen_maps(
        n0, h0=(0.25, 0.35), l0=(0.4, 0.6), s0=(0.5, 0.7)
    )
    # red
    cmap_fp, cmap_border_fp = gen_maps(n0, h0=(0, 0.1), l0=(0.4, 0.6), s0=(0.5, 0.7))
    # blue
    cmap_fn, cmap_border_fn = gen_maps(n0, h0=(0.6, 0.7), l0=(0.4, 0.6), s0=(0.5, 0.7))
