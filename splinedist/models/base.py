...
def generic_masked_loss(
    mask, n_params, loss, weights=1, norm_by_mask=True, reg_weight=0, reg_penalty=K.abs
):
...
def masked_loss_mae(mask, n_params, reg_weight=0, norm_by_mask=True):
    return masked_loss(
        mask, n_params, K.abs, reg_weight=reg_weight, norm_by_mask=norm_by_mask
    )


def masked_loss_mse(mask, n_params, reg_weight=0, norm_by_mask=True):
    return masked_loss(
        mask, n_params, K.square, reg_weight=reg_weight, norm_by_mask=norm_by_mask
    )


def masked_metric_mae(mask, n_params):
    def relevant_mae(y_true, y_pred):
        return masked_loss(mask, n_params, K.abs, reg_weight=0, norm_by_mask=True)(
            y_true, y_pred
        )
...
def masked_metric_mse(mask, n_params):
    def relevant_mse(y_true, y_pred):
        return masked_loss(mask, n_params, K.square, reg_weight=0, norm_by_mask=True)(
            y_true, y_pred
        )
...
class SplineDistBase(BaseModel):
    ...
    def prepare_for_training(self, optimizer=None):
        ...
        def relevant_mae(dist_true_mask, dist_pred):
            dist_true, dist_mask = split_dist_true_mask(dist_true_mask)
            return masked_metric_mae(dist_mask, n_params=self.config.n_params)(
                dist_true, dist_pred
            )

        def relevant_mse(dist_true_mask, dist_pred):
            dist_true, dist_mask = split_dist_true_mask(dist_true_mask)
            return masked_metric_mse(dist_mask, n_params=self.config.n_params)(
                dist_true, dist_pred
            )
    ...
    def predict(
        ...
        **predict_kwargs,
        ...
        (:class:`numpy.ndarray`,:class:`numpy.ndarray`)
            Returns the tuple (`prob`, `dist`) of per-pixel object probabilities and control point locations.
